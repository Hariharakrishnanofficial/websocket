...
httpServer.on('upgrade', (req, socket, head) => {
    const url = req.url || '/';
    // Accept "/" and "/ws" (and anything starting with /ws) for flexibility.
...
wss.on('connection', (ws, req) => {
    ...
    const registerTimer = setTimeout(() => {
        if (!ws.role) {
            ...
            try { ws.close(4000, 'registration timeout'); } catch {}
        }
    }, REGISTER_TIMEOUT);
    ...
    ws.on('message', (raw) => {
        ...
        if (!ws.role) {
            ...
            catch { try { ws.close(4001, 'invalid registration'); } catch {} return; }
            ...
            if (msg.type === 'robot') {
                ...
                try { ws.close(4001, 'unknown type'); } catch {}
            }
            return;
        }
